...
def gen_relative_position_index(
        ...
        k_size: Optional[Tuple[int, int, int]] = None,
        class_token: bool = False,
) -> torch.Tensor:
    # Adapted with significant modifications from Swin / BeiT codebases
    # get pair-wise relative position index for each token inside the window
    # Some ideas from 3D from here: https://github.com/nengwp/3D-MaxViT-pytorch/blob/main/maxvit3d.py
    ...
    coords = torch.stack(
        ndgrid(
            ...
            torch.arange(q_size[2]),
        )
    ).flatten(1)  # 3, Wh, Ww, Wd
    relative_coords = coords[:, :, None] - coords[:, None, :]  # 3, Whd*Ww, Whd*Ww
    relative_coords = relative_coords.permute(1, 2, 0)  # Qh*Qw, Kh*Kw, 3
    relative_coords[:, :, 0] += q_size[0] - 1  # shift to start from 0
    relative_coords[:, :, 1] += q_size[1] - 1
    relative_coords[:, :, 2] += q_size[2] - 1
    relative_coords[:, :, 0] *= (2 * q_size[1] - 1) * (2 * q_size[2] - 1)
    relative_coords[:, :, 1] *= (2 * q_size[2] - 1)
    num_relative_distance = (2 * q_size[0] - 1) * (2 * q_size[1] - 1) * (2 * q_size[2] - 1)
...
def resize_rel_pos_bias_table(
        rel_pos_bias,
        new_window_size: Tuple[int, int, int],
        new_bias_shape: Tuple[int, ...],
):
    ...
    dst_size = (new_window_size[0] * 2 - 1, new_window_size[1] * 2 - 1, new_window_size[2] * 2 - 1)
    print(rel_pos_bias.shape, new_window_size, new_bias_shape)
    if rel_pos_bias.ndim == 4:
        # TF maxvit style (num_heads, H, W, D) bias shape, no extra tokens currently supported
        num_extra_tokens = 0
        _, dst_h, dst_w = new_bias_shape
        assert dst_h == dst_size[0] and dst_w == dst_size[1]
        num_attn_heads, src_h, src_w = rel_pos_bias.shape
        ...
        dst_num_pos, _ = new_bias_shape
        src_num_pos, num_attn_heads = rel_pos_bias.shape
        num_extra_tokens = dst_num_pos - (dst_size[0] * dst_size[1] * dst_size[2])
        src_size = int((src_num_pos - num_extra_tokens) ** 0.5)
        src_size = (src_size, src_size, src_size)
        has_flat_shape = True

    if src_size[0] != dst_size[0] or src_size[1] != dst_size[1] or src_size[2] != dst_size[2]:
    ...
    rel_pos_bias = torch.repeat_interleave(rel_pos_bias, dst_size[2], dim=0)
    return rel_pos_bias
...
class RelPosBias(nn.Module):
    ...
    def __init__(self, window_size, num_heads, prefix_tokens=0):
        ...
        num_relative_distance = (2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2] - 1) + 3 * prefix_tokens
...
def gen_relative_log_coords(
        ...
        pretrained_win_size: Tuple[int, int, int] = (0, 0, 0),
        mode='swin',
):
    ...
    relative_coords_d = torch.arange(-(win_size[2] - 1), win_size[2], dtype=torch.float32)
    relative_coords_table = torch.stack(torch.meshgrid([relative_coords_h, relative_coords_w, relative_coords_d]))
    relative_coords_table = relative_coords_table.permute(1, 2, 3, 0).contiguous()  # 2*Wh-1, 2*Ww-1, 2*Wd-1, 3
    if mode == 'swin':
        if pretrained_win_size[0] > 0:
            relative_coords_table[:, :, 0] /= (pretrained_win_size[0] - 1)
            relative_coords_table[:, :, 1] /= (pretrained_win_size[1] - 1)
            relative_coords_table[:, :, 2] /= (pretrained_win_size[2] - 1)
        else:
            relative_coords_table[:, :, 0] /= (win_size[0] - 1)
            relative_coords_table[:, :, 1] /= (win_size[1] - 1)
            relative_coords_table[:, :, 2] /= (win_size[2] - 1)
...
def generate_lookup_tensor(
    ...
    # TODO: I'm sure it's incorrect but will work - need to fix after
    ret = torch.zeros(length, length, length, vocab_size)
...
def reindex_2d_einsum_lookup(
    ...
    """Reindex 3d relative position bias with 2 independent einsum lookups.
    ...
    """
    # print(relative_position_tensor.shape)
    reindexed_tensor = torch.einsum('nhwd,ixrh->nixwd', relative_position_tensor, height_lookup)
    # print(reindexed_tensor.shape)
    reindexed_tensor = torch.einsum('nixwd,jyrw->nijxyd', reindexed_tensor, width_lookup)
    # print(reindexed_tensor.shape)
    reindexed_tensor = torch.einsum('nijxyd,kzrd->nijkxyz', reindexed_tensor, depth_lookup)
    # print(reindexed_tensor.shape)
...
class RelPosBiasTf(nn.Module):
    ...
    def __init__(self, window_size, num_heads, prefix_tokens=0):
        ...
        vocab_depth = 2 * window_size[2] - 1
        self.bias_shape = (self.num_heads, vocab_height, vocab_width, vocab_depth)
        ...
        self.register_buffer('depth_lookup', generate_lookup_tensor(window_size[2]), persistent=False)
        self.init_weights()
    ...
    def get_bias(self) -> torch.Tensor:
        ...
        # print("GGG", self.height_lookup.shape, self.width_lookup.shape, self.depth_lookup.shape)
