...
from timm_3d.models._features import FeatureListNet, FeatureHookNet
from timm_3d.models._features_fx import FeatureGraphNet
from timm_3d.models._helpers import load_state_dict
from timm_3d.models._hub import has_hf_hub, download_cached_file, check_cached_file, load_state_dict_from_hf
from timm_3d.models._manipulate import adapt_input_conv
from timm_3d.models._pretrained import PretrainedCfg
from timm_3d.models._prune import adapt_model_from_file
from timm_3d.models._registry import get_pretrained_cfg
...
def convert_2d_weights_to_3d(state_dict, verbose=True):
    ...
    for layer in layers:
        if (
            ...
            or 'stem.0.weight' in layer
            or 'features' in layer
            or 'pre_logits' in layer
            or 'mlp.fc' in layer
        ):
    ...
    for n in default_kwarg_names:
        ...
        if n == 'img_size':
            ...
            if input_size is not None:
                ...
                kwargs.setdefault(n, input_size[-3:])
        elif n == 'in_chans':
            input_size = pretrained_cfg.get('input_size', None)
            if input_size is not None:
                assert len(input_size) == 4
...
def build_model_with_cfg(
    ...
    print('=============== BUILD MODEL WITH CONFIG ============')
    print('FEATURE CONFIG:', feature_cfg)
    print('PRETRAINED CONFIG:', pretrained_cfg)
    print('KWARGS:', kwargs)
    print('KWARGS FILTER:', kwargs_filter)
    _update_default_model_kwargs(pretrained_cfg, kwargs, kwargs_filter)
    print('KWARGS AFTER UPDATE:', kwargs)

    # Setup for feature extraction wrapper done at end of this fn
    # print("***DEBUG***", kwargs.get('features_only', None), feature_cfg, kwargs)
