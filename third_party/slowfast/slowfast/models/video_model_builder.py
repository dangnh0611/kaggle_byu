...
class ResNet(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        # stride: 1, 4, 8, 16, 32
        self.output_channels = [cfg.DATA.INPUT_CHANNEL_NUM, 256, 512, 1024, 2048]
    ...
    def forward(self, x, bboxes=None):
        ...
        assert len(x) == 1
        features.append(x[0])
        # print('after pathway:', [e.shape for e in y])
        x = self.s3(y)
        assert len(x) == 1
        features.append(x[0])
        x = self.s4(x)
...
class X3DEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        super(X3DEncoder, self).__init__()
        self.norm_module = get_norm(cfg)
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_pathways = 1
    ...
    def _construct_network(self, cfg):
        ...
        self.output_channels = [cfg.DATA.INPUT_CHANNEL_NUM[0], dim_res1]
        # blob_in = s1
        dim_in = dim_res1
        for stage, block in enumerate(self.block_basis):
            dim_out = round_width(block[1], w_mul)
            ...
            dim_in = dim_out
            self.output_channels.append(dim_out)
            self.add_module(prefix, s)
...
class MViTEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.downsample_block_idxs = []
        for i in range(len(cfg.MVIT.POOL_Q_STRIDE)):
            _blk_idx = cfg.MVIT.POOL_Q_STRIDE[i][0]
            _q_stride = cfg.MVIT.POOL_Q_STRIDE[i][1:]
            stride_q[_blk_idx] = _q_stride
            if any([e > 1 for e in _q_stride]):
                assert i != 0, 'First block has stride > 1, which is unexpected!'
                self.downsample_block_idxs.append(_blk_idx)
            if cfg.MVIT.POOL_KVQ_KERNEL is not None:
                pool_q[_blk_idx] = cfg.MVIT.POOL_KVQ_KERNEL
            else:
                pool_q[_blk_idx] = [
                    s + 1 if s > 1 else s for s in cfg.MVIT.POOL_Q_STRIDE[i][1:]
                ]
        ...
        input_size = self.patch_dims
        self.output_channels = [in_chans, embed_dim]
        self.feature_shapes = [self.input_dims, input_size]
        ...
        if self.enable_rev:
            ...
            for i in range(depth):
                ...
                if len(stride_q[i]) > 0:
                    ...
                    if i in self.downsample_block_idxs:
                        self.output_channels.append(dim_out)
                        self.feature_shapes.append(input_size)
    ...
    def patch_to_image(self, x, thw):
        B, D, C = x.shape
        if self.cls_embed_on:
            outputs = x[:, :, 1:] # trim cls token
            outputs = x.transpose(1,2).reshape(B, C-1, thw[0], thw[1], thw[2])
        else:
            outputs = x.transpose(1,2).reshape(B, C, thw[0], thw[1], thw[2])
        return outputs
    ...
    def forward(self, x, bboxes=None, return_attn=False):
        ...
        if self.enable_rev:
            ...
            for i, blk in enumerate(self.blocks):
                x, thw = blk(x, thw)
                # print(i, thw)
                if i+1 in self.downsample_block_idxs or i+1 == len(self.blocks):
                    # print('add', i, thw)
                    features.append(self.patch_to_image(x, thw))
