...
class VideoModelStem(nn.Module):
    ...
    def __init__(
        ...
        self._construct_stem(dim_in, dim_out, norm_module, stem_func_name, temporal_pool)

    def _construct_stem(self, dim_in, dim_out, norm_module, stem_func_name, temporal_pool = False):
...
class ResNetBasicStem(nn.Module):
    ...
    def __init__(
        ...
        self.bn_mmt = bn_mmt
        # Construct the stem layer.
        self._construct_stem(dim_in, dim_out, norm_module, temporal_pool)

    def _construct_stem(self, dim_in, dim_out, norm_module, temporal_pool = False):
        self.conv = nn.Conv3d(
            dim_in,
            dim_out,
            self.kernel,
        ...
        self.pool_layer = nn.MaxPool3d(
            kernel_size=[3 if temporal_pool else 1, 3, 3], stride=[2 if temporal_pool else 1, 2, 2], padding=[1 if temporal_pool else 0, 1, 1]
        )
