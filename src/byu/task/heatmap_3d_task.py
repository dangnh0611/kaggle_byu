...
class Heatmap3dTask(BaseTask):
    """
    Task: 3D UNet to predict 3D heatmap
    """
    ...
    def build_loss_func(self):
        from yagm.loss.hem import HardExampleMiningLossWrapper
        from yagm.loss.seg.multiscale import MultiscaleSegmentationLossWrapper

        loss_cfg = deepcopy(self.cfg.loss)
        if hasattr(loss_cfg, "mtl"):
            mtl_cfg = loss_cfg.mtl
            del loss_cfg.mtl
        else:
            mtl_cfg = None
        if hasattr(loss_cfg, "hem"):
            hem_cfg = loss_cfg.hem
            del loss_cfg.hem
        else:
            hem_cfg = None

        loss_func = hydra.utils.instantiate(loss_cfg)
        if hem_cfg is not None:
            loss_func = HardExampleMiningLossWrapper(loss_func, **hem_cfg)

        try:
            num_scales = len(self.cfg.model.head.ms)
        except:
            num_scales = 1
        ms_loss_func = MultiscaleSegmentationLossWrapper(
            loss_func=loss_func, num_losses=num_scales, mtl_cfg=mtl_cfg
        )
        return ms_loss_func
    ...
    def shared_step(self, model, stage, batch, batch_idx, dataloader_idx=None):
        ...
        ms_pred_heatmaps = model(batch_img.half())

        loss = self.loss_func(ms_pred_heatmaps, target_heatmap)
        ...
        if stage != "train":
            ...
            if torch.isnan(loss_dict["loss"]):
                ...
                _ms_pred_heatmaps = ms_pred_heatmaps
                ms_pred_heatmaps = []
                for pred_heatmap in _ms_pred_heatmaps:
                    torch.nan_to_num(pred_heatmap, nan=nan_fill, out=pred_heatmap)
                    ms_pred_heatmaps.append(pred_heatmap)
        ...
        if stage != "train":
            ...
            pred_heatmap = ms_pred_heatmaps[0]
