...
class Join3dTask(BaseTask):
    """
    Task: 3D UNet to jointly predict 3D heatmap, regression, DSNT.
    Not completed implementation, you should ignore this class.
    """
    ...
    def shared_step(self, model, stage, batch, batch_idx, dataloader_idx=None):
        ...
        target_heatmap = batch["heatmap"]
        assert batch_img.dtype == torch.uint8 and target_heatmap.dtype == torch.float16
        outs = model(batch_img.float())
        pred_kpt, pred_kptness, pred_heatmap, pred_dsnt_kpt = outs
        loss = self.loss_func(
            pred_kpt,
            batch["kpt_gt"],
            batch["kpt_mask"],
        ...
        if stage != "train":
            ...
            if torch.isnan(loss_dict["loss"]):
                assert not torch.isnan(batch_img).any()
                assert not torch.isnan(target_heatmap).any()
                nan_fill = (
                    float("-inf") if self.cfg.task.decode.act == "sigmoid" else 0.0
                )
                logger.critical(
                    "Nan prediction found in %s batch %d, attempt fill nan by %f",
                    stage,
                    batch_idx,
                    nan_fill,
                )
                torch.nan_to_num(pred_heatmap, nan=nan_fill, out=pred_heatmap)
        ...
        if stage != "train":
            ...
            step_output["pred_heatmap"] = pred_heatmap
            step_output["pred_kpt"] = pred_kpt
            step_output["pred_kptness"] = F.sigmoid(pred_kptness)
            step_output["pred_dsnt_kpt"] = pred_dsnt_kpt
        ...
        for channel_idx, heatmap in enumerate(pred_heatmap):
            ...
            if cfg.task.decode.method == "nms":
                ...
                outputs = decode_heatmap_3d(
                    ...
                    blur_operator=None,
                    conf_thres=cfg.data.label_smooth[0] + cfg.task.decode.conf_thres,
    ...
    def eval_step_single_model(
        ...
        patch_size_in = self.cfg.data.patch_size
        heatmap_stride = self.cfg.task.decode.heatmap_stride
        patch_size_out = [
            round(e / stride) for e, stride in zip(patch_size_in, heatmap_stride)
        ]
        step_output, step_metrics = self.shared_step(
            model, stage, batch, batch_idx, dataloader_idx=dataloader_idx
        )
        batch_pred_heatmap = step_output["pred_heatmap"]

        if self.cfg.task.decode.act == "sigmoid":
            batch_pred_heatmap = F.sigmoid(batch_pred_heatmap)
        elif self.cfg.task.decode.act == "softmax":
            # ignore first background channel
            batch_pred_heatmap = F.softmax(batch_pred_heatmap, dim=1)[
                :, 1:
            ]  # (B,C+1,...) -> (B,C,...)
        elif self.cfg.task.decode.act is None or self.cfg.task.decode.act == "identity":
            pass
        else:
            raise ValueError

        B, C, Z2, Y2, X2 = batch_pred_heatmap.shape
        assert C == 1
        # interpolate back to original (finest resolution)
        if [Z2, Y2, X2] != patch_size_out:
            logger.debug(
        ...
        model_result = self.val_pred_results.setdefault(
            model_name,
            {
                ...
                "heatmap_submission": SubmissionDataFrame(),
                "regress_submission": SubmissionDataFrame(),
                "dsnt_submission": SubmissionDataFrame(),
            },
        )
        for i in range(B):
            ...
            pred_patch_kpt = step_output["pred_kpt"][i, 0].tolist()
            pred_patch_kptness = step_output["pred_kptness"][i, 0].item()
            pred_patch_dsnt_kpt = step_output["pred_dsnt_kpt"][i, 0].tolist()

            # interpolate to match heatmap_stride

            tomo_shape = batch["tomo_shape"][i].cpu().tolist()
            tomo_heatmap_shape = [
                round(e / stride) for e, stride in zip(tomo_shape, heatmap_stride)
            ]
            ...
            if is_first:
                ...
                model_result["cur_kpts"] = []
                model_result["cur_dsnt_kpts"] = []
            ...
            cur_pred_kpts = model_result["cur_kpts"]
            cur_pred_dsnt_kpts = model_result["cur_pred_dsnt_kpts"]
            cur_heatmap_submission: SubmissionDataFrame = model_result["submission"]
            cur_regress_submission: SubmissionDataFrame = model_result[
                "regress_submission"
            ]
            cur_dsnt_submission: SubmissionDataFrame = model_result["dsnt_submission"]
            ...
            assert (
                tuple(
                    (pe - ps) // stride
                    for pe, ps, stride in zip(patch_end, patch_start, heatmap_stride)
                )
                == pred_patch_heatmap.shape[1:]
            )
            sz, sy, sx = tuple(rs - ps for rs, ps in zip(roi_start, patch_start))
            ez, ey, ex = tuple(
                ps - ce + pe for ps, ce, pe in zip(patch_size_in, patch_end, roi_end)
            )
            dst_slices = [
                slice(None),
                slice(
                    round((roi_start[0] + top_pad_z) / heatmap_stride[0]),
                    round((roi_end[0] - bot_pad_z) / heatmap_stride[0]),
                ),
                slice(
                    round((roi_start[1] + top_pad_y) / heatmap_stride[1]),
                    round((roi_end[1] - bot_pad_y) / heatmap_stride[1]),
                ),
                slice(
                    round((roi_start[2] + top_pad_x) / heatmap_stride[2]),
                    round((roi_end[2] - bot_pad_x) / heatmap_stride[2]),
                ),
            ]
            src_slices = [
                slice(None),
                slice(
                    round((sz + top_pad_z) / heatmap_stride[0]),
                    round((ez - bot_pad_z) / heatmap_stride[0]),
                ),
                slice(
                    round((sy + top_pad_y) / heatmap_stride[1]),
                    round((ey - bot_pad_y) / heatmap_stride[1]),
                ),
                slice(
                    round((sx + top_pad_x) / heatmap_stride[2]),
                    round((ex - bot_pad_x) / heatmap_stride[2]),
                ),
            ]
            ...
            pred_patch_kpt = [
                _kpt * _size + _start
                for _kpt, _size, _start in zip(
                    pred_patch_kpt, patch_size_in, patch_start
                )
            ] + [pred_patch_kptness]
            pred_patch_dsnt_kpt = [
                _kpt * _size + _start
            ...
            cur_pred_kpts.append(pred_patch_kpt)
            cur_pred_dsnt_kpts.append(pred_patch_dsnt_kpt)
            ...
            if is_last:
                ...
                for channel_idx, channel_outputs in enumerate(outputs):
                    ...
                    if len(channel_outputs) == 1:
                        ...
                        # one motor detected
                        cur_heatmap_submission.add_row(
                ...
                ########### DECODE REGRESSION ############
                # one motor detected
                cur_pred_kpts = sorted(cur_pred_kpts, key=lambda x: x[-1], reverse=True)
                z, y, x, conf = cur_pred_kpts[0]
                cur_regress_submission.add_row(
                    tomo_id=f"{tomo_id}@{tta_name}",
                    x=x * target_spacing[2] / ori_spacing[2] - 0.5,
                    y=y * target_spacing[1] / ori_spacing[1] - 0.5,
                    z=z * target_spacing[0] / ori_spacing[0] - 0.5,
                    conf=conf,
                )

                ########### DECODE DSNT ############
                # one motor detected
                cur_pred_dsnt_kpts = sorted(
                    cur_pred_dsnt_kpts, key=lambda x: x[-1], reverse=True
                )
                z, y, x, conf = cur_pred_dsnt_kpts[0]
                cur_regress_submission.add_row(
                    tomo_id=f"{tomo_id}@{tta_name}",
                    x=x * target_spacing[2] / ori_spacing[2] - 0.5,
                    y=y * target_spacing[1] / ori_spacing[1] - 0.5,
