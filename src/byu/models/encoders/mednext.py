...
class MedNeXt(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            conv(in_channels, n_channels, kernel_size=stem_stride, stride=stem_stride),
            LayerNorm(normalized_shape=n_channels, data_format="channels_first"),
        )
        if type(exp_r) == int:
        ...
        # @TODO - refactor
        # hard code here, follow by official implementation
        self.decoder_num_blocks = decoder_num_blocks
        if decoder_num_blocks >= 0:
        ...
        if decoder_num_blocks > 0:
            self.out_0 = OutBlock(
                in_channels=n_channels * (2 ** (4 - decoder_num_blocks)),
                n_classes=n_classes,
                dim=dim,
            )
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            if self.decoder_num_blocks < 0:
                ret.extend([x_res_0, x_res_1, x_res_2, x_res_3])
                return ret
...
class MedNeXtEncoder(nn.Module):
    ...
    def __init__(self, global_cfg):
        ...
        self.encoder: MedNeXt = create_mednext_v1(**encoder_kwargs)
        print("ENCODER:", self.encoder.output_channels)
...
if __name__ == "__main__":
    ...
    x = torch.zeros((1, 1, 224, 448, 448), requires_grad=False).cuda()
    flops = FlopCountAnalysis(model, x)
    print(flops.total())

    with torch.no_grad():
        print(model)
        x = torch.zeros((1, 1, 224, 448, 448)).cuda()
        out = model(x)
        print("OUTPUT SHAPE:", [e.shape for e in out])
