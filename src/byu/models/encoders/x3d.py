...
class X3DEncoder(nn.Module):
    ...
    def __init__(self, global_cfg):
        ...
        slowfast_cfg.merge_from_file(
            f"third_party/slowfast/configs/Kinetics/X3D_{cfg.encoder.model_size}.yaml"
        )
        slowfast_cfg.DATA.INPUT_CHANNEL_NUM = [3]
        slowfast_cfg.MODEL.NUM_CLASSES = 0
        slowfast_cfg.MODEL.ARCH = "x3dencoder"
        slowfast_cfg.MODEL.MODEL_NAME = "X3DEncoder"
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        B, C, D, H, W = x.shape
        # @TODO - properly handle normalization
        ...
        features = [e[0].permute(0, 1, 3, 4, 2) for e in features]  # BCDHW -> BCHWD
        return features
