...
EXP20_2D_CONVNEXT_TINY_ALLGTV3 = {
    "model": "EXP20_2D_CONVNEXT_TINY_ALLGTV3",
    "heatmap_stride": 16,
    "tta": ["yx"],
    # "tta": ["yx", "xy"],
    ...
    "fold_metas": [
        {
            "fold": 0,
            "config_path": "EXP20_2D_CONVNEXT_TINY_ALLGTV3_config.yaml",
            "ckpt_path": "EXP20_2D_CONVNEXT_TINY_ALLGTV3_ep=3_step=15002_val_Fbeta=0.952797_val_PAP=0.947485.ckpt",
            "ema": 0.99,
        },
    ],
}
...
JOBS = [
    {
        "sigs": [EXP20_2D_CONVNEXT_TINY_ALLGTV3],
        "names": ["EXP20_2D_CONVNEXT_TINY_ALLGTV3"],
        "weights": [1.0],
    },
]
...
class KFoldModelWrapper(nn.Module):
    ...
    def _forward_save_memory(self, x):
        ...
        for model_idx, model in enumerate(self.fold_models):
            out_heatmap = torch.nan_to_num(
                self.act(model(x)[2]), nan=0.0, posinf=1.0, neginf=0.0
            )  # (B, 1, D, H, W)
    ...
    def _forward_fast(self, x):
        ...
        out_heatmaps = [
            torch.nan_to_num(self.act(model(x)[2]), nan=0.0, posinf=1.0, neginf=0.0)
...
def decode_heatmap(pred_heatmap, target_spacing, stride, blur_operator=None):
    radius_voxels = [1000.0 / e / s for e, s in zip(target_spacing, stride)]
    radius_thres = [e / 3.0 for e in radius_voxels]
...
def inference_worker(
    ...
    if DECODE_NMS_BLUR_SIGMA is not None:
        ...
        sigma_voxels = [
            1000 / e / stride * DECODE_NMS_BLUR_SIGMA
            for e, stride in zip(TARGET_SPACINGS[0], HEATMAP_STRIDE)
        ]
        blur_operator = (
        ...
        while True:
            ...
            for i in range(B):
                for model_idx in range(num_models):
                    ...
                    pred_patch_heatmap = batch_heatmaps[model_idx][i]  # (C, H, W)
