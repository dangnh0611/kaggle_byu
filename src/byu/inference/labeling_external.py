...
##################################################
SIG_exp9_allfolds_x3dfactfpn = {
    "model": "X3D",
    "heatmap_stride": 16,
    # "tta": ["zyx"],
    # "tta": ["zyx_z", "zxy_z"],
    "tta": ["zyx", "zxy", "zyx_x", "zyx_y", "zyx_z"],
    ...
    "torch_combined_weight_path": "exp9_allfolds_x3dfactfpn/model.pth",
    "trt_combined_weight_path": "exp9_allfolds_x3dfactfpn/model.engine",
    "onnx_combined_weight_path": "exp9_allfolds_x3dfactfpn/model.onnx",
    "trt_precision": "fp16",
    ...
    "fold_metas": [
        # {
        #     "fold": 0,
        #     "config_path": "exp9_allfolds_x3dfactfpn/config.yaml",
        #     "ckpt_path": "exp9_allfolds_x3dfactfpn/fold0_ep=3_step=18000_val_Fbeta=0.953565_val_PAP=0.956512.ckpt",
        #     "ema": 0.99,  # _self_ has best Fbeta, but ema has better PAP/mPAP
        # },
        {
            "fold": 1,
            "config_path": "exp9_allfolds_x3dfactfpn/config.yaml",
            "ckpt_path": "exp9_allfolds_x3dfactfpn/fold1_ep=3_step=20000_val_Fbeta=0.948661_val_PAP=0.952866.ckpt",
            "ema": 0.99,  # _self_ has best Fbeta, but ema has better PAP/mPAP
        },
        # {
        #     "fold": 2,
        #     "config_path": "exp9_allfolds_x3dfactfpn/config.yaml",
        #     "ckpt_path": "exp9_allfolds_x3dfactfpn/fold2_ep=3_step=20000_val_Fbeta=0.944969_val_PAP=0.942273.ckpt",
        #     "ema": 0.99,  # _self_ has best Fbeta, but ema has better PAP/mPAP
        # },
        # {
        #     "fold": 3,
        #     "config_path": "exp9_allfolds_x3dfactfpn/config.yaml",
        #     "ckpt_path": "exp9_allfolds_x3dfactfpn/fold3_ep=3_step=18000_val_Fbeta=0.979325_val_PAP=0.988813.ckpt",
        #     "ema": 0.99,  # _self_ has best Fbeta, but ema has better PAP/mPAP
        # }
    ],
}
...
SIG_exp8_r101 = {
    ...
    # "tta": ["zyx", "zxy", "zyx_x", "zyx_y", "zxy_x", "zxy_y", "zyx_z", "zxy_z"],
    # "tta": ["zyx",  "zyx_x", "zyx_y","zyx_z", "zyx_xy", "zyx_xz", "zyx_yz", "zyx_xyz", "zxy", "zxy_x", "zxy_y", "zxy_z", "zxy_xy", "zxy_xz", "zxy_yz", "zxy_xyz"],
    "torch_combined_weight_path": "exp8_r101/model.pth",
    "trt_combined_weight_path": "exp8_r101/model.engine",
    "onnx_combined_weight_path": "exp8_r101/model.onnx",
    "trt_precision": "fp16",
    "patch_size": [224, 448, 448],
    "border": [0, 0, 0],
    "overlap": [0, 0, 0],
    ...
    "fold_metas": [
        {
            "fold": 0,
            "config_path": "exp8_r101/config.yaml",
            "ckpt_path": "exp8_r101/ep=3_step=20000_val_Fbeta=0.959128_val_PAP=0.957578.ckpt",
            "ema": 0.99,  # _self_ has best Fbeta, but ema has better PAP/mPAP
        }
    ],
}
...
SIG_exp17_convnext_tiny_2d = {
    "model": "SIG_exp17_convnext_tiny_2d 3D",
    "heatmap_stride": 16,
    # "tta": ["zyx"],
    # "tta": ["zyx_x", "zyx_y"],
    "tta": ["zyx", "zxy", "zyx_x", "zyx_y", "zyx_z"],
    ...
    # "tta": ["zyx", "zxy", "zyx_x", "zyx_y", "zxy_x", "zxy_y", "zyx_z", "zxy_z"],
    # "tta": ["zyx",  "zyx_x", "zyx_y","zyx_z", "zyx_xy", "zyx_xz", "zyx_yz", "zyx_xyz", "zxy", "zxy_x", "zxy_y", "zxy_z", "zxy_xy", "zxy_xz", "zxy_yz", "zxy_xyz"],
    "torch_combined_weight_path": "exp17_convnext_tiny_2d/model.pth",
    "trt_combined_weight_path": "exp17_convnext_tiny_2d/model.engine",
    "onnx_combined_weight_path": "exp17_convnext_tiny_2d/model.onnx",
    "trt_precision": "fp16",
    "patch_size": [224, 448, 448],
    "border": [0, 0, 0],
    "overlap": [0, 0, 0],
    ...
    "fold_metas": [
        {
            "fold": 0,
            "config_path": "exp17_convnext_tiny_2d/config.yaml",
            "ckpt_path": "exp17_convnext_tiny_2d/fold0_ep=3_step=20000_val_Fbeta=0.963252_val_PAP=0.948334.ckpt",
            "ema": 0.99,  # _self_ has best Fbeta, but ema has better PAP/mPAP
        },
    ],
}
...
JOBS = [
    {
        ...
        "names": ["ensemble_r101_x3d_convnexttiny2d"],
...
if MODE == "LOCAL":
    DEVICES = [0]
    ASSETS_DIR = "assets/"
    DATA_DIR = "/home/dangnh36/datasets/.comp/byu/processed/external/tomogram"
    WORKING_DIR = f"outputs/submit/working/external_labeling"
    VAL_GT_CSV_PATH = (
        "/home/dangnh36/datasets/.comp/byu/processed/external_labels_v1.csv"
    )
    ...
    for i, row in GT_DF.iterrows():
        _tomo2spacing[row["tomo_id"]] = row["target_spacing"]
    ALL_TOMO_SPACINGS = [_tomo2spacing[tomo_id] for tomo_id in ALL_TOMO_IDS]

# VALIDATION ON KAGGLE
elif MODE == "KAGGLE_VAL":
    DEVICES = [0, 1]
    ASSETS_DIR = "/kaggle/input/byu-checkpoints"
    DATA_DIR = "/kaggle/input/byu-locating-bacterial-flagellar-motors-2025/train"
    WORKING_DIR = "/kaggle/working/"
    VAL_GT_CSV_PATH = "/kaggle/input/byu-checkpoints/gt.csv"
    with open("/kaggle/input/byu-checkpoints/skf5_rd42.json", "r") as f:
        cv_meta = json.load(f)
...
class KFoldModelWrapper(nn.Module):
    def __init__(
        ...
        for fold_idx, meta in enumerate(fold_metas):
            ...
            # @TODO - hotfix only, remove plz
            cfg.task._target_ = "src.byu.task.heatmap_3d_task.Heatmap3dTask"
            task: BaseTask = l_utils.build_task(cfg)
            if not load_combined_weight:
                l_utils.load_lightning_state_dict(
                    model=task,
                    ckpt_path=os.path.join(base_dir, meta["ckpt_path"]),
                    cfg=cfg,
                )
    ...
    def _forward_save_memory(self, x):
        ...
        for model_idx, model in enumerate(self.fold_models):
            out_heatmap = self.act(model(x)[0])  # (B, 1, D, H, W)
    ...
    def _forward_fast(self, x):
        ...
        out_heatmaps = [self.act(model(x)[0]) for model in self.fold_models]
...
def decode_heatmap(pred_heatmap, target_spacing, stride, blur_operator=None):
    radius_voxel = 1000 / target_spacing / stride
    radius_thres = radius_voxel / 3.0
    ...
    pool_ksize = [3, 3, 3]
    logger.debug(
        "Heatmap decode with radius=%f, pool_ksize=%s, radius_thres=%f",
        radius_voxel,
        pool_ksize,
        radius_thres,
    )
...
def decode_segment_mask(
    ...
    radius_voxel_thres = 1000 / target_spacing / stride * radius_factor_thres
    volume_thres = np.pi * radius_voxel_thres * radius_voxel_thres
...
def tomo_producer_worker_func(
    ...
    while True:
        ...
        for spacing_idx, target_spacing in enumerate(target_spacings):
            print("before", tomo_idx, tomo_id, spacing_idx, tomo.shape)
            spaced_tomo = spacing_torch(
                tomo, ori_spacing, target_spacing, device=device, mode=mode
            )
            print(
                "spaced", tomo_idx, tomo_id, spacing_idx, tomo.shape, spaced_tomo.shape
            )
            proxy_tomos.append(
        ...
        logger.info(
            "[TOMO PRODUCER %s] Load tomo_idx=%d tomo_id=%s spacings %f -> %s with shape %s -> %s take %.2f sec",
            worker_id,
            tomo_idx,
            tomo_id,
            ori_spacing,
...
def inference_worker(
    ...
    if DECODE_NMS_BLUR_SIGMA is not None:
        ...
        sigma_voxel = 1000 / TARGET_SPACINGS[0] / HEATMAP_STRIDE * DECODE_NMS_BLUR_SIGMA
        print("BUR OPERATOR SIGMA:", sigma_voxel)
        blur_operator = (
        ...
        while True:
            ...
            for i in range(B):
                for model_idx in range(num_models):
                    ...
                    ori_spacing = float(batch["ori_spacing"][i].item())
                    target_spacing = float(batch["target_spacing"][i].item())
                    ...
                    if is_first:
                        logger.info(
                            "[INFER] Model %s receive first patch of tomo %s (index %d)",
                            model_idx,
                            tomo_id,
                            tomo_idx,
                        )
                        ...
                        assert abs(target_spacing - TARGET_SPACINGS[0]) < 1e-4
                    ### end IS_FIRST

                    assert tomo_idx == model_result["cur_tomo_idx"]
                    assert model_result["main_target_spacing"] <= target_spacing
                    ...
                    if is_last:
                        ...
                        logger.debug(
                            "[INFER] Receive last patch of tomo %s (index %d), decoding..",
                            tomo_id,
                            tomo_idx,
                        )
                        ...
                        for _channel_idx, keypoints in enumerate(outputs):
                            ...
                            keypoints[:, :3] = (keypoints[:, :3] + 0.5) * (
                                HEATMAP_STRIDE * main_target_spacing / ori_spacing
                            ) - 0.5
    ...
    # JOIN
    logger.info("Waiting for all worker to be finished..")
    all_workers = [
...
for job_idx, job in enumerate(JOBS):
    ...
    for tomo_idx, (tomo_id, ori_spacing) in enumerate(
        ...
        input_queue.put((tomo_idx, tomo_id, ori_spacing))
...
df = df.groupby("tomo_id").first()
df["tomo_id"] = df.index
df = df.reset_index(drop=True)

GT_DF = GT_DF.groupby("tomo_id").first()
GT_DF["tomo_id"] = GT_DF.index
GT_DF["num_motors"] = 1
GT_DF["voxel_spacing"] = GT_DF["target_spacing"]
...
df.loc[df["conf"] < DECODE_CONF_THRES, ["motor_z", "motor_y", "motor_x"]] = -1
if MODE in ["LOCAL", "KAGGLE_VAL"]:
