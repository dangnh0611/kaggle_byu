...
if MODE == "LOCAL":
    ...
    WORKING_DIR = f"outputs/submit/working/labeling_round2"
...
def decode_heatmap(pred_heatmap, target_spacing, stride, blur_operator=None):
    ...
    for channel_idx, heatmap in enumerate(pred_heatmap):
        outputs = decode_heatmap_3d(
            ...
            max_dets=8 if (VIZ_ENABLE and MODE != "KAGGLE") else 1,
            timeout=None,
        )
        outputs = outputs.cpu()
        ret.append(outputs)
...
all_names = ["ensemble_r101_x3d_convnexttiny2d_part0"]
dfs = []
