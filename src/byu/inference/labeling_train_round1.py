...
parser = argparse.ArgumentParser(description="Example script to parse an integer")
parser.add_argument("--fold", type=int, required=True, help="FOLD INDEX")

args = parser.parse_args()

FOLD_IDX = args.fold
print("LABELING ON FOLD", FOLD_IDX)
...
HEATMAP_ENSEMBLE = False
HEATMAP_AGG_MODE = "avg"
HEATMAP_AGG_LOGITS = False
...
JOBS = [
    {
        "sigs": [SIG_6_5folds_spacing16_bg02],
        "names": ["x3d"],
        "weights": [1.0],
    },
]
...
if MODE == "LOCAL":
    ...
    WORKING_DIR = f"outputs/submit/working/fold_{FOLD_IDX}"
    VAL_GT_CSV_PATH = "/home/dangnh36/datasets/.comp/byu/processed/gt.csv"
    with open(
        "/home/dangnh36/datasets/.comp/byu/processed/cv/skf5_rd42.json", "r"
    ) as f:
        cv_meta = json.load(f)
        ALL_TOMO_IDS = cv_meta["folds"][FOLD_IDX]["val"][:]  # fold 0
...
class KFoldModelWrapper(nn.Module):
    def __init__(
        ...
        for fold_idx, meta in enumerate(fold_metas):
            ...
            cfg.task._target_ = "src.byu.task.heatmap_task.HeatmapTask"
...
def inference_worker(
    ...
    if heatmap_ensemble:
        assert len(cache_results) == 1 and cache_results.keys()[0] == 0
