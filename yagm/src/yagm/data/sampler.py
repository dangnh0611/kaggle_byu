...
from yagm.utils.misc import local_numpy_temporary_seed
...
class WeightedInfiniteSampler(Sampler):
    ...
    def _infinite_indices(self):
        ...
        sampler = WeightedRandomSampler(
            self.weights, self._size, replacement=True, generator=g
        )
...
class BalanceSampler(Sampler):
    ...
    def __init__(
        ...
        logger.info(
            "[SAMPLER] num_pos=%d num_neg=%d pos_ratio=%f",
            self.num_pos,
            self.num_neg,
            round(self.ori_ratio, 6),
        )
        ...
        if self.num_linear_epochs > 0:
            ...
            _ratios = np.linspace(start_ratio, end_ratio, self.num_linear_epochs)
            ratios.extend(_ratios)

        if self.num_constant_epochs > 0:
            constant_ratio = (
                constant_ratio if constant_ratio is not None else self.ori_ratio
            )
        ...
        self.ratios = ratios
        logger.info(
            "[SAMPLER] positive ratio per epochs: %s",
    ...
    def set_epoch(self, ep):
        assert (
            ep < self.max_epochs
        ), f"[SAMPLER] Invalid set_epoch() with ep={ep} while max_epoch={self.max_epochs}"
        ...
        with local_numpy_temporary_seed(seed):
            epoch_idxs = self._compute_epoch_idxs(ratio, one_pos_mode=self.one_pos_mode)
        logger.info(
            "[SAMPLER] epoch=%d ratio=%f seed=%d samples=%d iters=%d",
            self.cur_epoch,
            ratio,
            seed,
            len(epoch_idxs),
    ...
    def _compute_epoch_idxs(self, ratio, one_pos_mode=True):
        if ratio < 0:
            ...
            logger.info("[SAMPLER] ratio=%f <0, use default RandomSampler..", ratio)
            n = len(self.dataset)
        ...
        epoch_num_iters = math.ceil((epoch_num_pos + self.num_neg) / self.batch_size)
...
class WeightedRandomSamplerWrapper(CustomWeightedRandomSampler):
    ...
    def __init__(self, dataset, replacement=True):
        super().__init__(
            weights=dataset.compute_sampling_weights(),
